...
authorname = input("What's your name ? (putting it into all trackinfo.ini)\n")
authorname = bytes(authorname, 'latin-1')

print("\nlook at this awesome tree promenade\n") 
dirlist = []
clearlist = ['English', 'French', 'Japanese', 'German', 'Italian', 'course_images', 'Spanish']

def rename(depth):
    print(os.getcwd(), depth)
    if depth == 0:
        for dirname in clearlist:
            if dirname in dirlist:
                dirlist.remove(dirname)
    for name in os.listdir('./'):
        if not os.path.exists(name) or name in ['$RECYCLE.BIN', 'System Volume Information']:
            continue
        ...
        if ext[-1] == '.arc':
            ...
            with open('trackinfo.ini', 'r+b') as ini:
                ini.write(b'[Config]\r\nauthor = ')
                ini.write(authorname)
                ini.write(b'trackname = \r\nreplaces = \r\nreplaces_music = \r\nmain_language = French')
                pointer = 0x29
                ini.seek(pointer)
                part2 = ini.read()
                ini.seek(pointer)
